...
Two plain Scala 3 modules, no macros: `core` (`Base`, `Other`) and `app`, which uses them.
...
# 4. add core/Added.scala (a new type) and use it from app/Use.scala
sbt compile                 # 5. incremental:                            [error] Not found: type Other / Base
```
...
[error] -- [E006] Not Found Error: app/src/main/scala/app/Use.scala:6:13
[error] 6 |  val other: Other[Base] = Other(List(Base("a")))
[error]   |             ^^^^^
[error]   |             Not found: type Other
[error] -- [E006] Not Found Error: app/src/main/scala/app/Use.scala:6:19
[error]   |           Not found: type Base - did you mean Byte.type? or perhaps Byte?
...
[error] 5 errors found
```

The new type `Added` resolves; every *unchanged* `core` type does not. A second `sbt compile`
...
$ unzip -l target/out/jvm/scala-3.9.0/core/early/core_3-0.1.0-SNAPSHOT.jar
     2506  core/Added.tasty
```

while `target/out/jvm/scala-3.9.0/core/classes` is complete: `Added.*` as real files next to
`Base.*` and `Other.*` as symlinks into the cache's CAS, which is how a cache hit materialises a
restored output directory. The classes and the packaged `core_3-0.1.0-SNAPSHOT.jar` are fine;
only the early jar is wrong.
...
directory is missing while `classes/` holds symlinks.
